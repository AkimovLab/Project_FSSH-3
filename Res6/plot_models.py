...
plot_params = {"figsize":[20, 5], "titlesize":24, "labelsize":28, "fontsize": 28, "xticksize":26, "yticksize":26,
               ...
               "plotting_option":0, "nac_idof":0, "margin_bottom":0.2, "margin_left":0.1 }

# Esch-Levine
#for indx, prms in enumerate([models.all_model_params[0]]):
#for indx, prms in enumerate(models.all_model_params[-1]):
for indx, prms in enumerate( [models.model_params4_0]):
    list_states = [x for x in range(prms["nstates"])]
    plot_params.update( { "prefix":F"HAM_{indx}" })
    dynamics_plotting.plot_surfaces(models.compute_model, [ prms ], list_states, -14.0, 14.0, 0.1, plot_params)
